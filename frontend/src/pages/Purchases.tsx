...
const categories = ['Matéria Prima', 'Estoque', 'Equipamento', 'Serviço', 'Outro'];
...
export default function Purchases() {
  ...
  const [form, setForm] = useState({ date: '', description: '', supplier: '', category: 'Outro', payment: 'Pix', value: '' });
  ...
  const saveMutation = useMutation({
    mutationFn: (item: any) => api.saveItem('purchases', item),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['data'] }); setShowForm(false); setEditing(null); },
  });

  const deleteMutation = useMutation({
    mutationFn: (id: string) => api.deleteItem('purchases', id),
  ...
  return (
    <div className="p-4 md:p-8">
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => { setEditing(null); setForm({ date: new Date().toISOString().slice(0, 10), description: '', supplier: '', category: 'Outro', payment: 'Pix', value: '' }); setShowForm(true); }}
          className="bg-emerald-500 hover:bg-emerald-600 text-white px-4 py-2 rounded-lg text-sm font-medium transition">+ Nova Compra</button>
      </div>
      ...
      <div className="space-y-2">
        {items.map((item: any) => (
          <div key={item.id} className="bg-gray-900 rounded-xl p-4 border border-gray-800 flex justify-between items-center hover:border-gray-700 transition">
            ...
            <div className="flex items-center gap-3">
              <p className="font-semibold text-orange-400">R$ {parseFloat(item.value || '0').toFixed(2)}</p>
              <button onClick={() => { setEditing(item); setForm({ date: item.date, description: item.description, supplier: item.supplier || '', category: item.category, payment: item.payment, value: item.value }); setShowForm(true); }}
        ...
        {!isLoading && items.length === 0 && <p className="text-gray-500 text-center py-8">Nenhuma compra registrada</p>}
      </div>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={() => setShowForm(false)}>
          <div className="bg-gray-900 rounded-2xl p-6 w-full max-w-md border border-gray-800 mx-4" onClick={e => e.stopPropagation()}>
            <h2 className="text-xl font-bold text-white mb-4">{editing ? 'Editar' : 'Nova'} Compra</h2>
